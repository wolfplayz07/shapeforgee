...
const KNOWN_PRIMITIVES = new Set(["box", "cylinder", "capsule", "ellipsoid", "frustum", "cone", "wedge"]);
...
export function inferPrimitiveKind(part) {
  const explicit = KNOWN_PRIMITIVES.has(part.kind) ? part.kind : null;
  if (explicit && explicit !== "box" && explicit !== "cylinder") return explicit;

  const name = normalize(part.name);
  const text = `${name} ${normalize(part.category)} ${normalize(part.purpose)}`;
  ...
  // Compact rounded parts recur across controls, joints, lighting, furniture, toys, and mechanisms.
  // Keep the cue set intentionally geometric rather than object-specific.
  if (hasAny(text, ["ball", "globe", "orb", "bulb", "knob", "spherical joint"]) && flatness < 3.2) {
    return "ellipsoid";
  }

  // Explicitly cylindrical names should stay round rather than becoming a generic rounded box.
  if (hasAny(name, ["wheel", "roller", "shaft", "axle", "hub", "rod", "pipe", "tube", "barrel", "bottle", "cup", "can", "stem"])) {
    return "cylinder";
  }
  ...
  if (hasAny(text, ["motor"])) return "cylinder";
